...
bool Kinematics::calcInverseKinematics(int to, Link target)
{
	...
	for(int n=0;n<iteration;n++){
		...
		if(to == RR2) ulink[RP4].q = -ulink[RP3].q;
		else if(to == LR2) ulink[LP4].q = -ulink[LP3].q;
		calcForwardKinematics(CC);
	}
	return false;
}
...
bool Kinematics::calcInverseKinematicsAll(Link RFLink, Link LFLink)
{
	...
	angle[KNEE_R1]		= -ulink[RP4].q;
	angle[KNEE_R2]		= -ulink[RP2].q;
	angle[LEG_PITCH_R]	= ulink[RP1].q + ulink[RP2].q;
	angle[LEG_ROLL_R]	= ulink[RR1].q;
	angle[LEG_YAW_R]	= ulink[RY].q;

	angle[FOOT_ROLL_L]	= ulink[LR2].q;
	angle[KNEE_L1]		= -ulink[LP4].q;
	angle[KNEE_L2]		= -ulink[LP2].q;
	angle[LEG_PITCH_L]	= ulink[LP1].q + ulink[LP2].q;
...
{
	// ToDo : 回転方向をチェック
	ulink[LR2].q =  angle[FOOT_ROLL_L];	// 左足ロール軸
	ulink[LP4].q = -angle[KNEE_L1    ];	// 左足膝下ピッチ軸（平行リンク）
	ulink[LP3].q =  angle[KNEE_L1    ];
	ulink[LP2].q = -angle[KNEE_L2    ];	// 左足膝上ピッチ軸（平行リンク）
	ulink[LP1].q =  angle[KNEE_L2    ] + angle[LEG_PITCH_L];					// 左股ピッチ軸
	ulink[LR1].q =  angle[LEG_ROLL_L ];	// 左股ロール軸
	ulink[LY ].q =  angle[LEG_YAW_L  ];	// 左股ヨー軸

	ulink[RR2].q =  angle[FOOT_ROLL_R];	// 右足ロール軸
	ulink[RP4].q = -angle[KNEE_R1    ];	// 右足膝下ピッチ軸（平行リンク）
	ulink[RP3].q =  angle[KNEE_R1    ];
	ulink[RP2].q = -angle[KNEE_R2    ];	// 右足膝上ピッチ軸（平行リンク）
	ulink[RP1].q =  angle[KNEE_R2    ] + angle[LEG_PITCH_R];					// 右股ピッチ軸
