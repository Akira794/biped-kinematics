...
/* plot by gnuplot */
PlotLink::PlotLink(Link *ulink){
	this->ulink[LINK_NUM] = ulink;
}
...
void  PlotLink::SetPlotConf(int roll, int yaw ){
...
double PlotLink::elm( Link *ulink, int root, int elem ){
	return ulink[root].p(elem);
}

void PlotLink::PlotLeg(double c_x, double c_y, double c_z, double theta ){
	#if 0
