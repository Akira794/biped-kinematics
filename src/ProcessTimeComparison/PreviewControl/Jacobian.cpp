...
{
	double M = calcTotalMass(ulink, CC);
	Matrix<double,3,1> MC = calcMC(ulink, CC); 
...
// 平行リンク向けヤコビアン計算
MatrixXd calcJacobian2(Link *ulink, vector<int> idx)
{
...
// GankenKunヤコビアン計算
MatrixXd calcJacobian_GankenKun(Link *ulink, vector<int> idx)
{
	size_t jsize = idx.size();
	Matrix<double,3,1> target = ulink[idx.back()].p;
	Matrix<double,6,6> J = MatrixXd::Zero(6,6);
