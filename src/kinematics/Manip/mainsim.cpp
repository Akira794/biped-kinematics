...
void SetDefaultAngle(Link* link)
{
  ...
  link[LLEG_J1].q = deg2rad(-30.0);
  link[LLEG_J2].q = deg2rad( 60.0);
  link[LLEG_J3].q = deg2rad( 0.0);
...
while(1)
	{
		...
		Target.R = kinema.RotationFromRPY( deg2rad(broll + roll), deg2rad(bpitch + pitch), deg2rad(byaw + yaw) );
